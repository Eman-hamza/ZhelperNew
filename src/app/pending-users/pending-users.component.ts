...
export class PendingUsersComponent implements OnInit{
  constructor(private admin:AdminService,private toast:ToastrService,public route:Router)
  {}
  patients:any[]=[]
  helpers:any[]=[]
  ngOnInit(): void {
...
ConfirmPatients(userId:any)
{
  ...
  this.admin.PendPatients(this.pending).subscribe({
    next: (beers) => {
      this.toast.success("Successuflly confirmed")
...
RejectPatients(userId:any){
  ...
  this.admin.PendPatients(this.pending).subscribe({
    next: (beers) => {
      this.toast.success("Successuflly rejected")
      this.ngOnInit();
